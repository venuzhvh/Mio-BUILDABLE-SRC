...
public class BlockUtil implements IMinecraftUtil
{
    ...
    public static List<BlockPos> getSphere(final float range, final boolean sphere, final boolean hollow) {
        ...
        for (int x = BlockUtil.mc.player.getPosition().getX() - (int)range; x <= BlockUtil.mc.player.getPosition().getX() + range; ++x) {
            for (int z = BlockUtil.mc.player.getPosition().getZ() - (int)range; z <= BlockUtil.mc.player.getPosition().getZ() + range; ++z) {
                for (int y = sphere ? (BlockUtil.mc.player.getPosition().getY() - (int)range) : BlockUtil.mc.player.getPosition().getY(); y < BlockUtil.mc.player.getPosition().getY() + range; ++y) {
                    final double distance = (BlockUtil.mc.player.getPosition().getX() - x) * (BlockUtil.mc.player.getPosition().getX() - x) + (BlockUtil.mc.player.getPosition().getZ() - z) * (BlockUtil.mc.player.getPosition().getZ() - z) + (sphere ? ((BlockUtil.mc.player.getPosition().getY() - y) * (BlockUtil.mc.player.getPosition().getY() - y)) : 0);
    ...
    public static List<Vec3d> getUnsafeBlocksFromVec3d(final Vec3d pos, final int height) {
        final ArrayList<Vec3d> vec3ds = new ArrayList<Vec3d>(4);
    ...
    public static List<Vec3d> getOffsetList(final int y) {
        final ArrayList<Vec3d> offsets = new ArrayList<Vec3d>(4);
    ...
    public static boolean placeBlock2(final BlockPos pos, final int slot, final boolean rotate, final boolean rotateBack, final boolean b) {
        if (TestUtil.isBlockEmpty(pos)) {
            ...
            for (final EnumFacing f : values) {
                ...
                final Vec3d hitVec = new Vec3d(pos.getX() + 0.5 + f.getXOffset() * 0.5, pos.getY() + 0.5 + f.getYOffset() * 0.5, pos.getZ() + 0.5 + f.getZOffset() * 0.5);
                if (!TestUtil.emptyBlocks.contains(neighborBlock) && BlockUtil.mc.player.getPositionEyes(BlockUtil.mc.getRenderPartialTicks()).distanceTo(hitVec) <= 4.25) {
    ...
    public static BlockResistance getBlockResistance(final BlockPos block) {
        ...
        if (BlockUtil.mc.world.getBlockState(block).getBlock().getBlockHardness(BlockUtil.mc.world.getBlockState(block), (World)BlockUtil.mc.world, block) != Float.intBitsToFloat(Float.floatToIntBits(-6.673269f) ^ 0x7F558B6B) && !BlockUtil.mc.world.getBlockState(block).getBlock().equals(Blocks.OBSIDIAN) && !BlockUtil.mc.world.getBlockState(block).getBlock().equals(Blocks.ANVIL) && !BlockUtil.mc.world.getBlockState(block).getBlock().equals(Blocks.ENCHANTING_TABLE) && !BlockUtil.mc.world.getBlockState(block).getBlock().equals(Blocks.ENDER_CHEST)) {
    ...
    public static boolean placeBlockSmartRotate(final BlockPos pos, final EnumHand hand, final boolean rotate, final boolean packet, final boolean isSneaking) {
        ...
        if (rotate) {
            Mio.rotationManager.lookAtVec3d(hitVec);
        }
        rightClickBlock(neighbour, hitVec, hand, opposite, packet);
        BlockUtil.mc.player.swingArm(EnumHand.MAIN_HAND);
        BlockUtil.mc.rightClickDelayTimer = 4;
